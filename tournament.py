...
class World:
    ...
    def generate_world(self):
        self.worldmap = [[ASCII_TILES["empty"]
    ...
    def buffer_worldmap(self):
        ...
        for obj in self.bullets + self.agents:
            self.worldmap_buffer[obj.position[1]
                                 ][obj.position[0]] = obj.ascii_tile
        for flag in self.flags:
            if not flag.agent_holding:
                self.worldmap_buffer[flag.position[1]
                                     ][flag.position[0]] = flag.ascii_tile
...
class AgentEngine:
        ...
        for y in range(0, max_distance*2+1):
            ...
            for x in range(0, max_distance*2+1):
                ...
                if x_world >= 0 and x_world < world.width and y_world >= 0 and y_world < world.height:
                    visible_world[-1].append(
                        world.worldmap_buffer[y_world][x_world])
                else:
    ...
    def control(self, world):
        action, direction = self.agent.update(self.get_visible_world(
            world), self.position, self.can_shoot, self.holding_flag)
        ...
        if action == "move":
            ...
            if direction == "right":
                world.bullets.append(Bullet(self, direction=(1, 0)))
            elif direction == "left":
                world.bullets.append(Bullet(self, direction=(-1, 0)))
            elif direction == "up":
                world.bullets.append(Bullet(self, direction=(0, -1)))
            elif direction == "down":
                world.bullets.append(Bullet(self, direction=(0, 1)))
